...
\only<+->{
\item Can we build predictive classifiers for this data?}
\only<+->{
...
\begin{frame}[fragile]{SVM - What is an SVM?}
\only<1->{
Works on linearily separable and nonseparable data of \textbf{arbitrary dimension}}
\begin{columns}[T,onlytextwidth]
\column{0.5\textwidth}
\begin{center}
\only<2->{
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_sep.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
    %  \includegraphics[height=0.3\textwidth]{svm_exp.png}
...
\only<3->{
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_nonsep.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
    %  \includegraphics[height=0.3\textwidth]{svm_exp.png}
...
\bigskip
\bigskip
The one that maximizes the hyperplane's distance from the nearest datapoints of either class while classifying all data correctly - \textbf{the margin}
\end{center}}
\column{0.55\textwidth}
\begin{center}
\only<+->{
...
\begin{center}
\only<+->{
Maximization of the margin can be formulated as a \textbf{constrained minimization problem}!

}
%\only<+->{
%The hyperplane is given by
%\begin{align*}
%g(\mathbf{x}) = \mathbf{w^Tx + b}
%\end{align*}}
%\only<+->{
%It turns out that the margin is given by $\frac{2}{||w||}$ and we maximize it by minimizing
%\begin{align*}
...
%\bigskip
%\bigskip
%With the constraint that \textbf{all data are classified correctly}.}
\end{center}
\column{0.55\textwidth}
\begin{center}
\begin{figure}
...
\begin{frame}[fragile]{SVM - What is an SVM? Linearily Nonseparable/Noisy Data}
\begin{columns}[T,onlytextwidth]
\column{0.5\textwidth}
\begin{center}
\only<+->{
...
\only<+->{
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_cl.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
    %  \includegraphics[height=0.3\textwidth]{svm_exp.png}
...
\begin{frame}[fragile]{SVM - What is an SVM? Linearily Nonseparable/Noisy Data}
\only<+->{
We permit errors, but still penalize them!}
\begin{columns}[T,onlytextwidth]
\column{0.5\textwidth}
\begin{center}
\only<+->{
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_nonsep_mar.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
    %  \includegraphics[height=0.3\textwidth]{svm_exp.png}
...
\only<+->{
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_nonsep_err.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
    %  \includegraphics[height=0.3\textwidth]{svm_exp.png}
...
%\column{0.5\textwidth}
%\only<+->{
%Instead of $\frac{1}{2}||\mathbf{w}||_2^2$ we now minimize
%\begin{align*}
%\frac{1}{2}||\mathbf{w}||_2^2 + C\sum_{i=1}^n \xi_i}
%\end{align*}
%\only<+->{
%where $\xi_i$ is the distance of a data point from their correct decision region.}
%
%\only<+->{
%\textbf{The parameter C lets us now control the tradeoff between the generalization and training error (overfitting).}}
%\column{0.5\textwidth}
%\begin{center}
%\begin{figure}
%      \includegraphics[height=0.8\textwidth]{svm_nonsep_err.png}
...
\column{0.5\textwidth}
\only<+->{
We can still formulate this as a constrained minimization problem, but now we \textbf{minimize the sum of errors while maximizing the margin}.}
\only<+->{

\bigskip
The coefficient $C$ controls the width of the margin at the expense of worse separation during training.}
\column{0.5\textwidth}
\begin{center}
\begin{figure}
      \includegraphics[height=0.8\textwidth]{svm_nonsep_err.png}
...
\only<+->{
\item Crossvalidation is performed to control for overfitting}
\end{itemize}
...
\begin{frame}[fragile]{SVM - Statistical Significance}
\begin{center}
\only<+->{
	\begin{figure}
      \includegraphics[width=1.0\textwidth]{statsig_0.png}
	\end{figure}}
\only<+->{
	\begin{figure}
      \includegraphics[width=1.0\textwidth]{statsig_1.png}
	\end{figure}}
	\end{center}
\end{frame}
...
\only<+->{
We want to quantify the similarity between two rank orders of dendrites $R$ and $Q$, which both have length $n$. For example, we would like to see whether similar dendrites react to similar stimuli.}

\only<+->{
\begin{table}
    \begin{tabular}{c|c|c|c|c}
      \toprule
      Dendrite & $R_i$ & $Q_i$ & $R_{acc}$ & $Q_{acc}$\\
      \midrule
      1 & 1 & 2 & 0.78 & 0.81\\
      2 & 2 & 1 & 0.72 & 0.76\\
      3 & 3 & 3 & 0.65 & 0.70\\
...
\begin{frame}[fragile]{Rank Order Correlation}
\only<+->{
Standard approach: \textbf{Spearman's rank order coefficient} 
$\boldsymbol{\rho}$}


\only<+->{
We use a modified version of that because we usually have \textbf{many uninformative dendrites} that can skew the result (their exact rank is unimportant).}
\end{frame}
...
\begin{frame}[fragile]{Rank Order Correlations of Dendrites Over Stimuli}
	...
	\caption*{Experiment 1}
      \includegraphics[width=1.0\textwidth]{rank_presence_0.png}
	\end{figure}}
	\end{center}
	\column{0.5\textwidth}
	\begin{center}
...
\begin{frame}[fragile]{Responses of One Dendrite to Different Stimuli}
	...
	\begin{figure}
	\caption*{Experiment 1}
      \includegraphics[width=1.0\textwidth]{tuning_ca2+_single_0.png}
	\end{figure}}
	\end{center}
	\column{0.5\textwidth}
	\begin{center}
	...
	\begin{figure}
	\caption*{Experiment 2}
      \includegraphics[width=1.3\textwidth]{tuning_ca2+single.png}
	\end{figure}}
	\end{center}
	\end{columns}
\end{frame}
...
\only<+->{
	\begin{figure}
      \includegraphics[width=1.0\textwidth]{statsig_t0.png}
	\end{figure}}
\only<+->{
	\begin{figure}
      \includegraphics[width=1.0\textwidth]{statsig_t1.png}
	\end{figure}}
	\end{center}
\end{frame}
...
\section{Multivariate SVM Analysis}

\begin{frame}[fragile]{Presence Detection - Feature Selection}
\only<+->{
Since we have fewer datapoints per condition (30-50 per class) than features (100-150), we are prone to overfit to noise (which is abundant in our data).}

\only<+->{
$\Rightarrow$ we have to do \textbf{feature selection}.}

\only<+->{
We can use the previously best discriminating dendrites.}
\end{frame}
...
\bibitem {mante} Mante V., Sussillo D., Shenoy, K. V., Newsome, W. T. Context-dependent computation by recurrent dynamics in prefrontal cortex. Nature 503, 78-84 (2013).
