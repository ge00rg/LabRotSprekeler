...
\begin{center}
\begin{figure}
      \includegraphics[height=0.6\textwidth]{svm_exp1.png}
	\end{figure}}
	%\only<+->{
	%\begin{figure}
...
SMVs...
\begin{itemize}
\item are a staple of machine learning
\item have a regularization parameter that can combat overfitting
\item are good for small samples with many features (in theory)
\end{itemize}
}
\end{columns}
	\begin{columns}[T,onlytextwidth]
...
\only<+->{
Problem: Many of the dendrites are not significant for classification but affect $\rho$ greatly. We would like to be able to \textbf{weight our} $\mathbf{R}$'s and $\mathbf{Q}$'s.}
\end{frame}
